...
function fetchConsortiumLibraries(consortium) {
	...
	setTimeout(function () {
		...
		$.getJSON(
			'https://api.kirjastot.fi/v4/library?lang=' + lang + '&consortium=' + consortium + '&limit=1500',
			function (data) {
...
function generateEventTimeDisplay(start, end) {
	...
	if (endDay != null) {
		...
		// Ends on different day and has ending time
			...
			endDayDisplay = '<img alt="" src="' + faPath + 'calendar.svg" class="fa-svg event-details-icon"> ' + endDay;
			endDayDisplay =
				endDayDisplay +
				'<img alt="" src="' +
				faPath +
...
function generateEventItem(event, id) {
	...
	if (locationData.length == 1) {
		...
		if (location.address != null && location.city != null && location.coordinates != null) {
			linkToNavigation = generateLinkToTransitInfo(
	...
	itemContent =
		'<div class="event-content"><div class="event-modal-description">' +
		itemContent +
	...
	var linkTitle =
		i18n.get('Read') + ': ' + itemTitle.replace(/&quot;/g, '') + ' ' + event.start_date.replace('00.00', '');
	...
	var listItem =
		...
		'<img alt="" src="' +
		faPath +
		'map-marker-alt.svg" class="fa-svg event-li-icon">' +
		eventLocation +
		'</span>' +
		'</div>' +
		'</a>' +
...
function generateFilters() {
	...
	if (window.innerWidth > 800) {
		var LocationFiltersHeight =
			$('.event-location-filter').innerHeight() + $('.event-category-filter').innerHeight() + 500;
...
function bindEventListEvents() {
	$('.event-item-link').on('click', function (e) {
		...
		var itemLocation =
			'<span class="event-detail event-location" aria-label="Location">' +
			'<img data-toggle="tooltip" title="' +
			i18n.get('Location') +
			'" data-placement="top" alt="" ' +
			'src="' +
		...
		var locationMetaContainer =
			'<div class="event-info-box event-location-info-box">' + itemLocation + locationInfo + transitInfo + '</div>';
